...
class Clogin extends Mdbhandler{
    ...
    public function  doLogin() {
        ...
        if($this->checklogin($username, $password)){
            ...
            return json_encode(array('load'=>'welcome'));
        }else{
    ...
    public function checklogin($username, $password){
        $result = $this->Query("SELECT * FROM USERS WHERE name_usuario = '$username' AND pass_usuario = '$password'");
        if($this->RecordsCount($result)>0){
            $row = $this->FetchAssoc($result);
            $this->uinfo['uid'] = base64_encode($row['name_usuario']);
